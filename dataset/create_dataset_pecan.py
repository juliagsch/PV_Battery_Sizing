...
""" Get all file names writen saved in txt file. """
...
def process_pair(args):
    ev_path, solar_file, load_file, op, ev = args
    ...
    try:
        ...
        if not (battery == "inf" or solar == "inf"):
            # Load solar and load traces
            ...
            traces = [None] * 17520
            traces[::2] = solar_trace
            traces[1::2] = load_trace

            line = traces + ev_data + [eue_target, battery, solar]
            if not ('inf' in line or 'naN' in line):
                with open(f"dataset_test_pecan.csv", 'a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow(line)
                with open(f"files_processed_pecan.csv", 'a', newline='') as file:
                    writer = csv.writer(file)
...
def get_test_load_files():
    files = []
    for type in ["austin", "california", "scaled"]:
        files = files + [f"{base_path}/data/test/load/{type}/{f}" for f in os.listdir(f"{base_path}/data/test/load/{type}") if f.endswith('.txt')]
    return files

if __name__ == "__main__":
    num_runs = 3

    test_load = get_test_load_files()
    test_solar = get_files(f"{base_path}/dataset/test_solar.txt")
    print("Number of Files available: Solar ", len(test_solar), " Load: ", len(test_load))
    ...
    for round_num in range(num_runs):
        ...
        for idx, load_file in enumerate(test_load):
            ...
            solar_file = test_solar[random.randint(0,len(test_solar)-1)]
            ev = EV(
            ...
            for op in policies:
                tasks.append((ev_path, solar_file, load_file, op, ev))
